        ...
        <div className="flex items-start gap-4">
          ...
          <div className="flex-1">
            ...
            <p className="text-sm text-silver-400 mb-4">
              Today I filtered 2 unsolicited calls, coordinated the pool maintenance crew, and scheduled tomorrow's HVAC inspection.
              One invoice requires your approval and the landscaping photos are ready for review.
            </p>
      ...
      <div className="grid grid-cols-3 gap-6">
        ...
        <div className="bg-surface border border-border rounded-xl p-6">
          ...
          <div className="space-y-3">
            ...
            {mockRequests.filter(r => r.status === 'completed').slice(0, 1).map((request) => (
              <div
                key={request.id}
        ...
        <div className="grid grid-cols-2 gap-3">
          {mockActivity.map((activity) => (
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <p className="text-sm text-text font-medium">{activity.title}</p>
                <p className="text-xs text-silver-500 mt-0.5 line-clamp-1">{activity.description}</p>
                <p className="text-xs text-silver-600 mt-1">{activity.timestamp}</p>
              </div>
              {activity.actionable && (
